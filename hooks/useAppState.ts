...
export const useAppState = () => {
  ...
  const startProblemPhase = useCallback(() => {
    const randomIndex = Math.floor(Math.random() * problemsData.problems.length);
  ...
  const updateUserSolution = useCallback((solution: string) => {
    setState({
      ...state,
  ...
  const addDocument = useCallback((document: Omit<Document, 'id' | 'visible' | 'createdAt'>) => {
  ...
  const toggleDocumentVisibility = useCallback((id: string) => {
    setState({
      ...state,
      documents: state.documents.map(doc => 
  ...
  const addNotification = useCallback((notification: Document) => {
    setState({
      ...state,
