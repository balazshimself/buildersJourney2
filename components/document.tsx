...
import { EyeIcon, Trash2Icon, EditIcon, SaveIcon } from "lucide-react";
...
export function Document({
  ...
  const { isDragging, handleMouseDown, startDragging, stopDragging } =
    useDocumentDrag({
      document,
      onPositionChange,
    });
        ...
        {isEditing ? (
          ...
          <div className="prose prose-sm max-w-none">
            {document.content.split("\n").map((line, i) => {
              ...
              if (line.startsWith("# ")) {
                return (
                  <h1 key={i} className="text-xl font-bold mt-2 mb-3">
                    {line.substring(2)}
                  </h1>
                );
              } else if (line.startsWith("## ")) {
                return (
                  <h2 key={i} className="text-lg font-semibold mt-2 mb-2">
                    {line.substring(3)}
                  </h2>
                );
              } else if (line.startsWith("### ")) {
                return (
                  <h3 key={i} className="text-md font-medium mt-2 mb-1">
                    {line.substring(4)}
                  </h3>
                );
              } else if (line.startsWith("- ")) {
