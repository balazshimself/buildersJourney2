...
export function DocumentPhase({
  ...
  // Monitor for new documents with metadata from "Accept" button
  useEffect(() => {
    // Only find accepted documents (not denied ones)
    const newResearchDocs = documents.filter(
      (doc) =>
        doc.type === "market-research" &&
        doc.metadata &&
        doc.metadata.cost !== undefined &&
        !doc.metadata._processed &&
        doc.metadata.accepted
    );

    newResearchDocs.forEach((doc) => {
      // Only process projects that are accepted and have a cost
      if (doc.metadata?.cost && !doc.metadata.accepted) {
        // Mark as processed so we don't apply the cost multiple times
        onUpdateDocument(doc.id, {
          metadata: {
            ...doc.metadata,
  ...
  useEffect(() => {
    const inProgressDocs = documents.filter(
      (doc) =>
        ...
        !doc.editable &&
        doc.countdown === undefined &&
        doc.metadata?.accepted // Skip denied research
    );
